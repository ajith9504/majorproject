...
parser.add_argument("--configGA", help="Config for the GA",
                    default="configGA.csv")
parser.add_argument(
    "--run", help="Iteration number of EvoPruneDeepTL", type=int)
...
a_guardar = str(tam_pop)+str(tipo)+str(input_size) + \
    "-" + str(extractor) + "-" + str(iteracion)
print(a_guardar)
...
if type_model == 0:  # neuronas
    ...
    fitness = Fitness(input_size=input_size, first_layer=primera, second_layer=segunda, data=datos,
                      layer=tipo, num_layers=info_layers, optimType=optimization_type, extractor=extractor)
...
f_pop = f + datos[0] + str(tam_pop)+str(tipo)+str(input_size) + \
    "-"+str(extractor)+"-"+str(iteracion)+".txt"
print(f_pop)
...
min_val_idxs = [x for x in range(
    len(hamming_distances)) if hamming_distances[x] == min(hamming_distances)]
print(min_val_idxs)
...
if tipo == "first":
    ...
    print('RBF Kernel CKA, between best and closest: {}'.format(
        kernel_CKA(weights_best, weights_closest)))
    print('RBF Kernel CKA, between best and best: {}'.format(
        kernel_CKA(weights_best, weights_best)))
else:
    ...
    print("CKA closest layer 1: {}".format(
        kernel_CKA(weights_best[0], weights_closest[0])))
    print("CKA closest layer 1: {}".format(
        kernel_CKA(weights_best[0], weights_best[0])))

    print("CKA closest layer 2: {}".format(
        kernel_CKA(weights_best[1], weights_closest[1])))
    print("CKA closest layer 2: {}".format(
        kernel_CKA(weights_best[1], weights_best[1])))
...
if tipo == "first":
    print("Hamming entre best y no pruning " +
          str(distance.hamming(mejor, sin_pruning)))
    ...
    print('RBF Kernel CKA, between best and no pruned: {}'.format(
        kernel_CKA(weights_best, weights_nopruning)))
    print('RBF Kernel CKA, between best and best: {}'.format(
        kernel_CKA(weights_best, weights_best)))
else:
    print("Hamming entre best y no pruning " +
    ...
    print("CKA no pruned layer 1: {}".format(
        kernel_CKA(weights_best[0], weights_nopruning[0])))
    print("CKA no pruned layer 1: {}".format(
        kernel_CKA(weights_best[0], weights_best[0])))

    print("CKA no pruned layer 2: {}".format(
        kernel_CKA(weights_best[1], weights_nopruning[1])))
    print("CKA no pruned layer 2: {}".format(
        kernel_CKA(weights_best[1], weights_best[1])))
