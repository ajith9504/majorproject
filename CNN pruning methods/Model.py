...
class EvoPruningModel(object):
    def __init__(self, train_generator, test_generator, info,
        ...
        self.model_to_prune = create_pruneable_model(
            self.input_shape, info['num_classes'])
        ...
        if len(self.prune_layers) == 1:
            ...
            model_to_prune, callbacks = prunF(
                self.model_to_prune, self.prune_layers, self.pruning_conf)
            ...
            model_to_prune, callbacks = prunF(
                self.model_to_prune, [self.prune_layers[0]], self.pruning_conf_fc1)
            model_to_prune, callbacks = prunF(
                model_to_prune, [self.prune_layers[1]], self.pruning_conf_fc2)
    ...
    def train(self, epochs=26):
        ...
        log.DebugInfo(
            f"Model train loss = {loss:.3f} and accuracy = {acc:.3f}")
    ...
    def weight_prune(self, k):
        ...
        for layer in self.raw_model.layers:
            if layer.name in self.prune_layers:
                ...
                b = len(weights[0][weights[0] == 0.0]) + \
                    len(weights[1][weights[1] == 0.0])
                pruned_perc[layer.name] = b/a*100
                log.DebugInfo(
                    f"{b/a*100:.2f}% of the weights on layer {layer.name} were pruned")
        return pruned_perc
    ...
    def test(self):
        ...
        log.DebugInfo(
            f"Model test loss = {res[0]:.3f} and accuracy = {res[1]:.3f}")
    ...
    def decay_prun_perc(self):
        ...
        for x, layer in enumerate(self.model.layers):
            if layer.name.split('_')[0] == "prune":
                ...
                log.DebugInfo(
                    f"{b/a*100:.2f}% of the weights on layer {layer.name} (layer index = {x}) were pruned")
        return output

    def __str__(self):
        """ Print number of pruned neurons on each pruneable layer """
        log.DebugWarning(
            "This is an approximation making the asumption that pruned weights are equal to 0.0")
    ...
    def save(self, path='./summary/model.h5'):
        ...
        tf.keras.models.save_model(strip_pruning(
            self.model), path, include_optimizer=False)
    ...
    def save_raw_model(self, path='./summary/model.h5'):
        ...
        tf.keras.models.save_model(
            self.raw_model, path, include_optimizer=False)
